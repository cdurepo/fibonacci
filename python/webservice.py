...
class get_fib(tornado.web.RequestHandler):
    ...
    @gen.coroutine
    def get(self, length):
        result = yield self.hello_world(int(length))
        result=tornado.escape.json_encode(result)
        self.write(result)
    @gen.coroutine
    def message(self,length):
        #text = 'message is',length
        print "local_high:",local_high
        hello_world(length)
        raise gen.Return(str(text))

    @gen.coroutine
    def hello_world(self,length):
        try:
             length = int(length)
        except:
            return jsonify(response="error", message="invalid reqeust, reqeust must be an integer")
        ...
        if (length > 0):
            if ((length <= local_high) or ((length > local_high) and (rdb.exists(length)))):
                #print "current high",high
                result = self.create_output(length)
                result = {'response':'success','output':result}
                raise gen.Return(str(result))

            else:
                return {'response':'error', 'message':'Not calculated yet please try again later'}
        else:
                return jsonify(response="error",message="invalid reqeust")
    ...
    def create_output (self,length):
            ...
            #for the given position
        global local_high
        print "local_high in output:",local_high
        if length <= local_high:
            fib_read.seek(0)
            print "before read"
            ...
            print "after read"
            yield fib_output
            print "fib_output in output:",fib_output
        else:
            # If the length is more than we have in the cache file_name
            # We need to update the cache file and then return the whole thing.
            while (local_high < length):
        ...
        raise gen.Return(str(fib_output))
